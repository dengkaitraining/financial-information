...
本開發環境具備高擴充性、多資料庫隔離、獨立組態與跨平台 (Windows / Linux / macOS) 相容特性。最新版本中已整合**台股公司基本資料搜尋、儲存與定時排程更新功能**。藉由非同步 Celery 背景任務與前端動態 3 秒輪詢機制，徹底解決了高耗時爬蟲下的 HTTP 連線逾時（502 Proxy Error）與高頻連線被 block 的痛點。
...
| **Celery Worker (`celery-worker`)**| Celery `5.6.3` | 背景非同步任務執行器。負責 yfinance / GNews 爬取、翻譯與技術分析落庫。 | 由 Django API 觸發非同步任務派發，於背景異步執行。 |
| **Celery Beat (`celery-beat`)** | django-celery-beat `2.6.0` | 定期排程調度器。配合 Unfold 設定，每 4 小時自動執行更新任務。 | Unfold 後台圖形化管理，定時驅動 `update_all_scheduled_stocks`。 |
...
1. Shell 腳本維持 Unix `LF` 換行格式，且執行單元測試 `docker compose exec fin-backend python manage.py test stock_db` 全數通過。
