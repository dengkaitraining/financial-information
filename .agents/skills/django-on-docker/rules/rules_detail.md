...
| **手動測試資料隱蔽** | 藉由 `SHOW_BACKEND_VER` 參數，測試開發環境（True）顯示 `backend_ver` 軟連結；正式上線環境（False）隱蔽並刪除該軟連結，僅留實體隱藏目錄 `.backend_ver` | `backend/entrypoint.sh`, `backend/` | 容器初始化啟動與執行手動測試驗證時 |
